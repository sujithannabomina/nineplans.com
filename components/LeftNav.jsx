...
export default function LeftNav() {
  ...
  // Avoid Next.js build/prerender issues with useSearchParams by reading query on client.
  useEffect(() => {
    try {
            ...
            {navItems.map((it) => (
              <Link
                ...
                className={cn(
                  ...
                  (it.href === "/" && pathname === "/" && tab === "latest") || (it.href.includes("tab=") && it.href.endsWith(tab))
                    ? "bg-black text-white"
                    : ""
                )}
              >
        ...
        {user ? (
          <div className="card p-3">
            ...
            <div className="space-y-1">
              <Link href="/saved" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm no-underline hover:bg-black hover:text-white transition">
                <Bookmark className="h-4 w-4" /> Saved (soon)
              </Link>
              <Link href="/profile/settings" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm no-underline hover:bg-black hover:text-white transition">
                <Settings className="h-4 w-4" /> Settings
              </Link>
              <Link href="/submit" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm no-underline hover:bg-black hover:text-white transition">
