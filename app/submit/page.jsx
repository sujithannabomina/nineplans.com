...
export default function SubmitPage() {
  ...
  // Resolve identity names from userDoc
  const realName    = userDoc?.name || user?.displayName || "";
  const aliasName   = userDoc?.alias || "";
  const anonPersona = userDoc?.anonName || "";

  // Alias must be set to post as alias
  const needsAlias = identity === "alias" && !aliasName;

  // What the current identity hint shows
  function getIdentityHint() {
    if (identity === "realname") return realName ? `Posting as "${realName}"` : "Set your name in settings";
    if (identity === "alias") {
      if (!aliasName) return "⚠️ No alias set — go to Profile Settings to add one";
      return `Posting as "${aliasName}" — profile hidden`;
    }
  ...
  async function handleSubmit(e) {
    ...
    if (needsAlias)     return setError("You haven't set an alias yet. Go to Profile Settings to add one.");
    ...
    try {
      ...
      // Resolve display name for this post
      let authorAlias;
      if (identity === "realname")  authorAlias = realName || user.displayName || "User";
      else if (identity === "alias") authorAlias = aliasName;
      else authorAlias = anonPersona || "Anonymous"; // anonymous uses persona or fallback
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {IDENTITY_OPTIONS.map(({ key, icon: Icon, label, desc, activeColor, iconColor }) => (
                <button
                  key={key}
                  type="button"
                  onClick={() => setIdentity(key)}
                  className={`rounded-xl border p-3 text-left transition ${
                    identity === key ? activeColor : "border-white/20 hover:border-white/40"
                  }`}
                >
                  <Icon className={`h-5 w-5 mb-2 ${identity === key ? iconColor : "text-white/40"}`} />
            ...
            <div className={`mt-3 rounded-xl px-3 py-2 text-xs flex items-center gap-2 ${
              ...
              <span className="flex-1">{getIdentityHint()}</span>
              {needsAlias && (
          ...
          <div className="flex gap-3">
            ...
            <button type="submit" disabled={submitting || needsAlias}
