...
// ✅ This export was missing in your build log
...
// Name + Phone update (Email is read-only)
...
export async function createPost({
  ...
  // increment category postCount if category exists
  // best-effort only
  try {
...
export function listenFeed({ mode = "latest", categorySlug = null }, cb) {
  ...
  if (categorySlug) {
    ...
    // simple trending = order by upvotes desc then createdAt
    qy = query(
      collection(db, "posts"),
      where("status", "==", POST_STATUS.ACTIVE),
...
export async function votePost(uid, postId, value) {
  ...
  if (next === 1) {
    ...
    // remove like activity doc if exists
...
export async function reportPost({ uid, postId, reason, details = "" }) {
  ...
  // mark post under review (best effort)
...
/* -----------------------------
  PROFILE LISTS (Likes/Saves/Comments/Shares)
-------------------------------- */
