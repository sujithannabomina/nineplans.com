...
export const refs = {
  ...
  commentVote: (postId, commentId, uid) => doc(db, "posts", postId, "comments", commentId, "votes", uid),
...
export function listenPost(postId, cb) {
  return onSnapshot(refs.post(postId), (snap) => cb(snap.exists() ? ({ id: snap.id, ...snap.data() }) : null));
}

export function listenComments(postId, cb) {
  const qy = query(refs.comments(postId), where("status", "==", "active"), orderBy("createdAt", "asc"), limit(COMMENT_LIMIT));
  return onSnapshot(qy, (snap) => cb(snap.docs.map((d) => ({ id: d.id, ...d.data() }))));
}

export async function createPost({ categorySlug, categoryName, title, body, tags, authorUid, authorAlias, authorPhoto }) {
  const now = Date.now();
  const docData = {
    categorySlug,
    categoryName,
  ...
  await updateDoc(refs.category(categorySlug), { postCount: increment(1) });
  return postRef.id;
}
...
export async function reportPost({ postId, categorySlug, reporterUid, reason, note }) {
  const now = Date.now();
  const r = await addDoc(refs.reports(), {
    targetType: "post",
    targetId: postId,
    categorySlug,
    reporterUid,
  ...
  // Safety: put post under review (ads hidden in UI)
  await updateDoc(refs.post(postId), { status: "under_review", updatedAt: now });
  return r.id;
}

export async function listOpenReports(limitN = 100) {
  const qy = query(refs.reports(), where("status", "==", "open"), orderBy("createdAt", "desc"), limit(limitN));
  const snap = await getDocs(qy);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

export async function closeReport(reportId, reviewerUid) {
...
export async function adminSetPostStatus(postId, status) {
  await updateDoc(refs.post(postId), { status, updatedAt: Date.now() });
}

export async function updateAlias(uid, alias) {
  await updateDoc(refs.user(uid), { primaryAlias: alias.trim().slice(0, 20) });
}
